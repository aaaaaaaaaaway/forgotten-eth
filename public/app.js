...
const EXCHANGES = {
  ...
  ahoolee_token_sale: {
    name: 'Ahoolee Token Sale',
    desc: 'Ahoolee\'s September 2017 ICO. The sale collected ~581 ETH against a 3,030 ETH soft cap and finalized in refund mode. 191 ETH still sits in the contract waiting for the long tail of stragglers — 107 of 323 contributors haven\'t called refund() yet. Single-tx claim: refund() on the contract returns your original ETH deposit.',
    color: '#0d9488',
    contract: '0x575cb87ab3c2329a0248c7d70e0ead8e57f3e3f7',
    deployed: 'September 2017',
    balanceAbi: 'function saleBalanceOf(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'saleBalanceOf',
  ...
  th_token_sale: {
    name: 'TruckHash Token Sale',
    desc: 'TruckHash\'s 2018 ICO missed its soft cap; owner enabled refundAllowed mode. Per-user balance lives in weiBalances(addr). Public refund() returns the deposit; no further admin action needed.',
    color: '#0d9488',
    contract: '0xcd806502ad2f9aeb32e23f8d647341d4b568201d',
    deployed: 'March 2018',
    balanceAbi: 'function weiBalances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'weiBalances',
    withdrawAbi: 'function refund()',
    withdrawArgs: () => [],
    withdrawCall: 'refund',
    category: 'ico',
  ...
  jincor_token_ico: {
    name: 'Jincor Token ICO',
    desc: 'Jincor\'s late-2017 ICO did not reach soft cap; refund mode is automatic. Per-user balance is tracked in deposited(addr); public refund() returns the deposit and emits Refunded(holder, amount).',
    color: '#0d9488',
    contract: '0xb3b33f59174f2ef62167770e4c9cabaa3879eb5d',
    deployed: 'November 2017',
    balanceAbi: 'function deposited(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'deposited',
  ...
  agrotechfarm_crowdsale: {
    name: 'AgroTechFarm Crowdsale',
    desc: 'AgroTechFarm\'s 2018 crowdsale failed; the owner switched state to Refunding. Per-user balance is in balances(addr). Public refund() returns the deposit while the contract is in Refunding state.',
    color: '#0d9488',
    contract: '0x3fd30f3e1fbf4f3ea6bdf3e3bb11826266708869',
    deployed: 'April 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  luckchemy_crowdsale: {
    name: 'Luckchemy Crowdsale',
    desc: 'Luckchemy\'s 2018 ICO finished without hitting soft cap. Per-user balance is in deposits(addr); public refund() returns the deposit.',
    color: '#0d9488',
    contract: '0x18777aec0b231d1a4a9c66b51253088a03affdfc',
    deployed: 'April 2018',
    balanceAbi: 'function deposits(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'deposits',
    withdrawAbi: 'function refund()',
    withdrawArgs: () => [],
    withdrawCall: 'refund',
    category: 'ico',
  ...
  arbitrage_eth_staking: {
    ...
    color: '#7c2d12',
    contract: '0x5eee354e36ac51e9d3f7283005cab0c55f423b23',
    deployed: 'August 2018',
    balanceAbi: 'function ethBalanceOf(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'ethBalanceOf',
  ...
  fomo3d_short_v2: {
    name: 'FoMo3Dshort v2',
    desc: 'An independent FoMo3D Short clone deployed July 20, 2018, distinct from the canonical 0x52083b1a deploy. Standard PlayerBook key-purchase / countdown / dividend mechanics. ~137 ETH remained in player vaults. Note: isHuman modifier.',
    category: 'gambling',
    color: '#ef4444',
    contract: '0x0ad3227eb47597b566ec138b3afd78cfea752de5',
    deployed: 'July 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  fomo3d_long_v2: {
    name: 'FoMo3Dlong v2',
    desc: 'A FoMo3D Long clone deployed August 6, 2018, distinct from the canonical 0xA62142 deploy. Partially insolvent: the largest player vault exceeds the contract balance and cannot be withdrawn, but ~110 smaller players can claim ~7.7 ETH combined. Note: isHuman modifier.',
    category: 'gambling',
    color: '#ef4444',
    contract: '0xe336d0c8b24d8e20b6f714522f590abadb601493',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  fomojp: {
    name: 'FoMoJP',
    desc: 'A Japanese-themed FoMo3D fork from July 31, 2018. Standard plyr_/getPlayerInfoByAddress/withdraw ABI. ~76 ETH still in player vaults. Note: isHuman modifier.',
    category: 'gambling',
    color: '#f97316',
    contract: '0xcb47c89cb17c10b719fc5ed9665bae157cac2cb1',
    deployed: 'July 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  f3dplus: {
    name: 'F3DPLUS',
    desc: 'A FoMo3D variant deployed July 22, 2018, branding itself as a "PLUS" version with the standard PlayerBook mechanics. ~35 ETH in player vaults. Note: isHuman modifier.',
    category: 'gambling',
    color: '#fb923c',
  ...
  snowstorm_a: {
    name: 'SnowStorm',
    desc: 'A FoMo3D fork deployed July 29, 2018 with the standard key-buy / countdown / vault mechanics. ~35 ETH in player vaults. Note: isHuman modifier.',
    category: 'gambling',
    color: '#0ea5e9',
    contract: '0xda8d7ff0d043848a689125e2c7ab87b16a0cbe81',
    deployed: 'July 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  snowstorm_b: {
    name: 'SnowStorm B',
    desc: 'A second SnowStorm deployment (July 29, 2018) with identical bytecode to the primary contract. ~25 ETH in player vaults. Note: isHuman modifier.',
    category: 'gambling',
    color: '#0ea5e9',
    contract: '0xb2b30d39074c52a60283a26f238abff31fcb4217',
    deployed: 'July 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  fomo: {
    name: 'Fomo',
    desc: 'A minimally-named FoMo3D fork deployed August 20, 2018. Standard PlayerBook ABI and isHuman gating. ~15 ETH in player vaults.',
    category: 'gambling',
    color: '#ef4444',
    contract: '0xaff69c67f5dbbdd088ccbc6d47cb9e0ea547e132',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  cryptophunks_market: {
    name: 'CryptoPhunks Marketplace',
    desc: 'CryptoPunks-style marketplace deployed December 2021 for the CryptoPhunks NFT collection (a flipped-pixel parody by NotLarvaLabs). Sale proceeds and outbid bids accumulate in pendingWithdrawals; sellers and outbid bidders call withdraw() to pull their ETH out. ~107 ETH still waiting.',
    category: 'nft',
    color: '#f43f5e',
    contract: '0xd6c037be7fa60587e174db7a6710f7635d2971e7',
    deployed: 'December 2021',
    balanceAbi: 'function pendingWithdrawals(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'pendingWithdrawals',
    withdrawAbi: 'function withdraw()',
  ...
  cryptocats_market: {
    name: 'CryptoCats Marketplace',
    desc: 'A late-2017 marketplace contract for the CryptoCats NFT collection — one of the early CryptoPunks clones. Sale proceeds and outbid bids accrue in pendingWithdrawals; sellers and outbid bidders call withdraw() (no args). ~45 ETH still in the contract.',
    category: 'nft',
    color: '#fb923c',
  ...
  cryptocats_v1: {
    name: 'CryptoCats v2',
    desc: 'March 2018 CryptoCats deployment (contractVersion==2, predecessor of the v3 marketplace at 0x19c320b4). withdraw() drains your pendingWithdrawals balance from buyCat() sales. The contract holds ~44 ETH but only ~9 ETH is user-claimable; the rest accumulated from getCat() calls in 2021 and is permanently stuck due to a bug v3 explicitly fixed.',
    category: 'nft',
    color: '#f59e0b',
  ...
  ld3d_official: {
    name: 'LD3D Official',
    desc: 'An August 2018 FoMo3D fork branded "LD3D Official" — standard PlayerBook key-buy mechanics with plyr_/getPlayerInfoByAddress/withdraw. ~87 ETH remained in player vaults. Note: isHuman modifier — only EOAs can withdraw.',
    category: 'gambling',
    color: '#a855f7',
    contract: '0x47663541167ece0b96d9e5c60f9e470b2a20f598',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  bingo4beast_1: {
    ...
    desc: 'An August 2018 FoMo3D-derivative game (B4B token). Non-standard player enumeration (plyr_(uint256) reverts with a custom guard) but the rest of the FoMo3D ABI works — getPlayerInfoByAddress + withdraw. ~120 ETH still in player vaults.',
    category: 'gambling',
    color: '#7c3aed',
    contract: '0x05aa2fdf9f58b426b49900834cce0565d88e52eb',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  bingo4beast_2: {
    name: 'Bingo4Beast Long Official (deploy 2)',
    desc: 'Second deployment of the Bingo4Beast B4B game (August 2018). Partially insolvent: only ~57% of contract balance (56 ETH) is recoverable across 571 holders — claim amounts wei-exact verified via Tenderly trace, since getPlayerInfoByAddress returns inflated round-mask values that exceed actual withdraw() payouts.',
    category: 'gambling',
    color: '#7c3aed',
    contract: '0x4fb7d68e0116f35ade131b6535b2db1027bf7650',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
    balanceTransform: (result) => result[5],
    withdrawAbi: 'function withdraw()',
    withdrawArgs: () => [],
    withdrawCall: 'withdraw',
  },
  fomo3d_ultra: {
    name: 'FoMo3D Ultra',
    desc: 'August 2018 FoMo3D-derivative "Ultra" variant (unverified source). withdraw() works for individual player vaults — ~163 holders can claim ~131 ETH combined. The remaining ~338 ETH lives in round-end jackpot/community pots not mapped to per-user balances. Claim amounts wei-exact verified via Tenderly.',
    category: 'gambling',
    color: '#a855f7',
    contract: '0xab83d96de35bad6f234178fbb6507203488e9626',
    deployed: 'August 2018',
    balanceAbi: 'function getPlayerInfoByAddress(address) view returns (uint256, bytes32, uint256, uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getPlayerInfoByAddress',
  ...
  spankchain_auction: {
    name: 'SpankChain Auction',
    desc: 'December 2017 Dutch auction for the SPANK token (the SpankChain adult-entertainment platform). 463 ETH still in the contract — 100% recoverable across 105 buyers; the largest single buyer is owed ~287 ETH. Source unverified, so per-address refund amounts are wei-exact computed via debug_traceCall on each holder.',
    category: 'ico',
    color: '#ec4899',
    contract: '0x4d1886daf2617cbd9e27abfd0f18a54f04f33c41',
    deployed: 'December 2017',
    balanceAbi: 'function allBuyers(address) view returns (uint256, uint256, uint256, uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'allBuyers',
    balanceTransform: (result) => result[0] - result[1],
    withdrawAbi: 'function withdraw()',
    withdrawArgs: () => [],
    withdrawCall: 'withdraw',
  },
  presalepool_f058: {
    name: 'Presale Pool (LINO/HEALP/Bulleon)',
    desc: 'An early-2018 ICO presale pool (unverified) holding ~91 ETH from 65 contributors plus tokens from Lino Network, Etheal, and Bulleon. Per-contributor refund computed wei-exact via debug_traceCall.',
    category: 'ico',
    color: '#10b981',
    contract: '0xf058ee35f381a12b3a0f504025419dcaf047ce7f',
    deployed: 'January 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_48c1: {
    name: 'Presale Pool 0x48c128',
    desc: 'An early-2018 ICO presale pool (unverified) holding ~98 ETH for 7 contributors. withdrawAll() returns each contributor\'s full deposit; 100% recoverable, wei-exact verified via debug_traceCall.',
    category: 'ico',
    color: '#10b981',
    contract: '0x48c128eafc3b937fb1f97889ad174ac02b4952cb',
    deployed: 'March 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_ff2c: {
    name: 'Presale Pool 0xff2c68',
    desc: 'A May 2018 ICO presale pool (unverified) holding 50 ETH for one large contributor. withdrawAll() returns the full deposit; 100% recoverable.',
    category: 'ico',
    color: '#10b981',
    contract: '0xff2c689cb750f2a7d46e7471521189ced948f965',
    deployed: 'May 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  sawgamespass: {
    name: 'SAW Games Pass',
    desc: 'A 2022 NFT mint contract (verified ERC-721) where the contract owner accumulated 39 ETH in mint proceeds and never called withdraw(). 1 holder (the owner) can recover 100%.',
    category: 'nft',
    color: '#f59e0b',
    contract: '0x347e3513ca6d5118cb2df3bc386eade1e8f25ceb',
    deployed: 'March 2022',
    balanceAbi: 'function balanceOf(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balanceOf',
  ...
  presalepool_b990: {
    name: 'Presale Pool 0xb9906c',
    desc: 'An April 2018 ICO presale pool (unverified) — small remaining balance after most contributors withdrew. 2.5 ETH recoverable across 2 holders via withdrawAll().',
    category: 'ico',
    color: '#10b981',
    contract: '0xb9906cf500d5e61b9673115151104c990a804dcb',
    deployed: 'April 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_d677: {
    name: 'Presale Pool 0xd6770a',
    desc: 'A mid-2018 ICO presale pool (unverified) holding ~37 ETH across 23 contributors. withdrawAll() returns each contributor\'s deposit; 100% recoverable, wei-exact verified.',
    category: 'ico',
    color: '#10b981',
    contract: '0xd6770aac65a48f5715889a4cdbf30a4faf73a073',
    deployed: 'August 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_796d: {
    name: 'Presale Pool 0x796dbc',
    desc: 'A 2018 ICO presale pool (unverified) holding ~35 ETH for 3 contributors. withdrawAll() returns each deposit; 100% recoverable.',
    category: 'ico',
    color: '#10b981',
    contract: '0x796dbc51b37f2d92b0af1271fd9520c8efd1be61',
    deployed: 'August 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_50c1: {
    name: 'Presale Pool 0x50c19f',
    desc: 'A 2018 ICO presale pool (unverified) holding ~34 ETH for 2 large contributors. withdrawAll() returns each deposit; 100% recoverable.',
    category: 'ico',
    color: '#10b981',
    contract: '0x50c19ffdbda85d1a07c09f01e113a94923801d32',
    deployed: 'August 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  presalepool_6f40: {
    name: 'Presale Pool 0x6f40d9',
    desc: 'A 2018 ICO presale pool (unverified) holding ~28 ETH across 285 contributors — the largest by holder count in this batch. withdrawAll() returns each deposit; 100% recoverable.',
    category: 'ico',
    color: '#10b981',
    contract: '0x6f40d96713b60d5083bc2493251907ca673de1b8',
    deployed: '2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  ambassadors_fund: {
    ...
    desc: 'A 2019 OpenZeppelin PaymentSplitter holding 17.5 ETH across 12 fund-share holders. release(address) pays each beneficiary their pro-rata share. Pass your own address to release().',
    category: 'ico',
    color: '#10b981',
    contract: '0xe9426198aec621203ba1fe07cf292b3796ba6248',
    deployed: '2019',
    balanceAbi: 'function shares(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'shares',
    withdrawAbi: 'function release(address account)',
    withdrawArgs: (user) => [user],
    withdrawCall: 'release',
  },
  just_hodl_it: {
    name: 'JustHodlIt',
    desc: 'A 2018 ICO contribution pool (unverified). 8 contributors can claim 16 ETH via withdrawAll() (78% — the rest is admin/fee buckets, not user-claimable).',
    category: 'ico',
    color: '#10b981',
    contract: '0x344285b2082d9c7981241dcc160beca6057637e6',
    deployed: '2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  game_fair: {
    name: 'GameFair',
    desc: 'A 2018 gambling/redemption contract (unverified). Partially insolvent — 4 holders can claim 14 ETH against 13 ETH of contract balance via redeem(). Late redeemers will revert.',
    category: 'gambling',
    color: '#a855f7',
    contract: '0x86ab844a067094ec2c5a4bade21fcf2aeee32122',
    deployed: '2018',
    balanceAbi: 'function balanceOf(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balanceOf',
    withdrawAbi: 'function redeem()',
    withdrawArgs: () => [],
    withdrawCall: 'redeem',
  },
  ibc_lottery: {
    name: 'IBC Lottery',
    desc: 'An August 2018 IBC-themed lottery game (unverified). 8 holders can claim 12.4 ETH via withdraw() — 97% coverage.',
    category: 'gambling',
    color: '#a855f7',
    contract: '0xc5aafe85775fe97bee49d16cc4f38aaf8dbf20c5',
    deployed: 'August 2018',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  contribution_pool: {
    ...
    desc: 'A 2017 contribution pool holding 10.6 ETH for one large contributor. withdraw() returns the full deposit.',
    category: 'ico',
    color: '#10b981',
    contract: '0xa812137eff2b368d0b2880a39b609fb60c426850',
    deployed: '2017',
    balanceAbi: 'function balances(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'balances',
  ...
  ethertoken_v2: {
    name: 'Neufund EtherToken v2',
    desc: 'Neufund deployed a second ETH-T wrapper in November 2018 for its post-ICBM LockedAccount migration. ETH-T is WETH-style: holders can burn their token balance through withdraw(uint256) to receive the same amount of native ETH. This entry tracks directly withdrawable holder balances only; LockedAccount-held ETH-T is tracked separately under Neufund.',
    category: 'ico',
    color: '#0d9488',
    contract: '0x0b7dc5a43ce121b4eaaa41b0f4f43bba47bb8951',
  ...
  lido_anchor_vault: {
    ...
    desc: 'Lido\'s stETH→Anchor Protocol (Terra) integration, shut down after the May-2022 Terra/LUNA collapse via Lido DAO Proposal 29. bETH (bonded ETH) holders on Ethereum can redeem 1:1 for stETH by calling withdraw() — no approval needed, as the vault holds the bETH burner role. Balances are read live from the bETH token and are stETH-denominated (~1:1 with ETH). bETH locked in the Wormhole bridge and held by exchanges or other contracts is excluded as non-claimable.',
    category: 'defi',
    color: '#00a3ff',
  ...
  p4d: {
    name: 'P4D',
    desc: 'A 2018 PoWH3D/P3D bridge wrapper. Holders accrue ETH "subdividends" from the contract\'s underlying P3D position, claimed directly via withdrawSubdivs. Note: its separate dividendsOf pool is denominated in P3D tokens, not ETH, so the recoverable balance is subdividendsOf.',
    category: 'gambling',
    color: '#dc2626',
    contract: '0x96a4ed03206667017777f010dea4445823acb0fc',
    deployed: '2018',
    balanceAbi: 'function subdividendsOf(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'subdividendsOf',
    withdrawAbi: 'function withdrawSubdivs(bool)',
    withdrawArgs: () => [true],
    withdrawCall: 'withdrawSubdivs',
  },
  unkoin: {
    name: 'UnKoin',
  ...
  dx_2: {
    name: 'DutchX dx_2',
    desc: 'DutchX was Gnosis\'s original on-chain batch auction DEX. This proxy still holds direct internal WETH/SAI balances and unclaimed auction proceeds for users who never completed the old claim + withdraw flow.',
    category: 'defi',
    color: '#009cb4',
  ...
  delphi_redemption: {
    name: 'Delphi RedemptionContract',
    desc: 'Delphi launched the DEL token in 2017 and later deployed a fixed-rate redemption contract. DEL holders can approve the redemption contract, then call redeemTokens(amount) to burn DEL for native ETH at 1/2410 ETH per DEL.',
    category: 'ico',
    color: '#7c3aed',
    ...
    noWalletCheck: true,
    multiStep: true,
    redemptionTokenAddress: '0x02b9806a64cb05f02aa8dcc1c178b88159a61304',
    redemptionTokenSymbol: 'DEL',
    withdrawTokenBalanceField: 'token_balance_wei',
    withdrawAbi: 'function redeemTokens(uint256 _amount)',
    withdrawArgs: (amount) => [amount],
    withdrawCall: 'redeemTokens',
  },
  ...
  abyss_daico_fund: {
    ...
    desc: 'Abyss DAICO fund is latched in CrowdsaleRefund state after the failed 2018 sale. contributors(address) tracks each remaining ETH deposit and refundCrowdsaleContributor() returns msg.sender\'s own contribution.',
    color: '#7c3aed',
    ...
    balanceArgs: (user) => [user],
    balanceCall: 'contributions',
    withdrawAbi: 'function refundCrowdsaleContributor()',
    withdrawArgs: () => [],
    withdrawCall: 'refundCrowdsaleContributor',
    category: 'ico',
  },
  bayesin_bys_crowsale: {
    name: 'Bayesin (BYS) CrowSale',
    desc: 'Bayesin BYS crowdsale missed its goal. fundBalance(address) tracks contributors and safeWithdrawal() lets each contributor reclaim their own ETH after the deadline.',
    color: '#0d9488',
    contract: '0xe8b1b40f2d307bce891833f46eef1f69560e6926',
    deployed: 'March 2018',
    balanceAbi: 'function fundBalance(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'fundBalance',
    withdrawAbi: 'function safeWithdrawal()',
    withdrawArgs: () => [],
    withdrawCall: 'safeWithdrawal',
    category: 'ico',
  },
  confideal_campaign: {
    name: 'Confideal Campaign',
    desc: 'Confideal campaign is in failed stage. contributions(address) tracks remaining contributor deposits and withdrawRefund() returns msg.sender\'s refundable ETH.',
    color: '#ef4444',
  ...
  crowdsale_4363b5: {
    ...
    desc: 'Failed soft-cap crowdsale with remaining returnETher(address) refund ledgers. releaseEthers() returns msg.sender\'s refundable ETH after the sale-failed condition.',
    color: '#f97316',
    contract: '0x4363b5d64f228c819dc706889b09a0dc76e22fb0',
    deployed: '2018',
    balanceAbi: 'function returnETher(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'returnETher',
    withdrawAbi: 'function releaseEthers()',
    withdrawArgs: () => [],
    withdrawCall: 'releaseEthers',
    category: 'ico',
  },
  crowdsale_c699d9: {
    name: 'Crowdsale (0xc699d9)',
    desc: 'Failed soft-cap crowdsale with balances(address) refund ledgers. refund() returns msg.sender\'s refundable ETH after the ICO end/soft-cap failure.',
    color: '#f97316',
  ...
  crowdsale_e82056: {
    ...
    desc: 'Failed funding-goal crowdsale. balanceOf(address) tracks remaining contributor balances and safeWithdrawal() returns msg.sender\'s own ETH after the deadline.',
    color: '#06b6d4',
  ...
  crowdsalewatch: {
    name: 'CrowdsaleWatch',
    desc: 'CrowdsaleWatch missed its funding goal. balanceOf(address) tracks the remaining contributor balance and safeWithdrawal() returns the refundable ETH.',
    color: '#06b6d4',
  ...
  gateway_ico_3091d3: {
    ...
    desc: 'Gateway ICO failed its post-end goal check. balances(address) tracks current refundable principal and refund() returns msg.sender\'s balance.',
    color: '#8b5cf6',
  ...
  i2presale: {
    name: 'I2 Presale',
    desc: 'I2 presale missed its funding goal. balanceOf(address) tracks contributors and safeWithdrawal() returns msg.sender\'s remaining ETH.',
    color: '#06b6d4',
  ...
  mahala_coin_ico: {
    ...
    desc: 'Mahala Coin crowdsale failed soft cap. balances(address) tracks current refundable principal and refund() returns msg.sender\'s balance after the sale end.',
    color: '#22c55e',
  ...
  reservation2: {
    name: 'Reservation2',
    desc: 'Reservation2 holds remaining per-user ETH in balanceOf(address). withdraw() returns msg.sender\'s balance with no extra refund gate.',
    color: '#eab308',
  ...
  singulardtv_launch: {
    ...
    desc: 'SingularDTV launch reached EndedAndGoalNotReached. contributions(address) tracks ETH deposits; users must approve the launch contract for sentTokens(address), then call withdrawContribution().',
    color: '#ec4899',
    ...
    balanceCall: 'contributions',
    multiStep: true,
    tokenApproval: { tokenCall: 'singularDTVToken', amountCall: 'sentTokens', spender: 'self' },
    tokenApprovalAbi: 'function singularDTVToken() view returns (address)',
    tokenAmountAbi: 'function sentTokens(address) view returns (uint256)',
    withdrawAbi: 'function withdrawContribution()',
    withdrawArgs: () => [],
    withdrawCall: 'withdrawContribution',
    category: 'ico',
  },
  startmining_stm_sale: {
    name: 'Start Mining (STM) Token Sale',
    desc: 'Start Mining STM token sale is finished with token goal unmet. getInvestorsETH(address) tracks refundable ETH and manualRefund() returns msg.sender\'s remaining deposit.',
    color: '#84cc16',
    contract: '0xe8da050c3140183d4f5f01e048ac136e2da5253f',
    deployed: '2018',
    balanceAbi: 'function getInvestorsETH(address) view returns (uint256)',
    balanceArgs: (user) => [user],
    balanceCall: 'getInvestorsETH',
    withdrawAbi: 'function manualRefund()',
    withdrawArgs: () => [],
    withdrawCall: 'manualRefund',
    category: 'ico',
  },
  gro_ust_comp: {
...
const V2_TOKEN_DECIMALS = { DAI: 18, SAI: 18, USDC: 6, USDT: 6, WBTC: 8, WETH: 18, B20: 18, MEME: 8, MUSE: 18, RARI: 18, WHALE: 4, NFTX: 18, MANA: 18, SAND: 18 };
...
async function populateV2SubCounters() {
  ...
  const setCell = (key, label) => {
    ...
    let claimed = parseFloat(t.claimed) || 0;
  ...
  for (var cKey in claimedBalances) {
    ...
    if (claim.claims && claim.claims.length > 1) {
      ...
      for (var ci = 0; ci < sorted.length; ci++) {
        var it = sorted[ci];
        // Prefer the human name (ENS deed labels like 'lottery.eth') over the
        // truncated labelhash when the API enriched it.
        var itLabel = it.name
          ? it.name + '.eth'
          : (it.item_id ? it.item_id.slice(0, 10) + '...' : '#' + (ci + 1));
...
async function checkUserBalances(overrideAddress) {
  ...
  // Generic approve -> redeem token-redemption flows — preload the
  // redemption-token balance + allowance to the protocol contract. Mirrors
  // _digixState. Anonymous address lookups skip this (no wallet to sign with
  // anyway), but the API still carries token_balance_wei so the card can show
  // the exact token amount needed.
  window._tokenRedemptionState = {};
  for (const { key } of balanceResults) {
    const cfg = EXCHANGES[key];
    if (!cfg.redemptionTokenAddress || !cfg.withdrawTokenBalanceField || !walletProvider) continue;
    try {
      const tokenC = new ethers.Contract(cfg.redemptionTokenAddress, ['function balanceOf(address) view returns (uint256)', 'function allowance(address,address) view returns (uint256)'], walletProvider);
      const tokenBal = await tokenC.balanceOf(checkAddr);
      const tokenAllowance = await tokenC.allowance(checkAddr, cfg.contract);
      window._tokenRedemptionState[key] = { tokenBal, tokenAllowance };
    } catch (e) {
      console.warn('Failed to read token redemption state for', key, e.message || e);
    }
  }

  // v2 ERC20 basket redemption flows — preload the redemption-token balance +
  ...
  for (const { key, balance } of balanceResults) {
    ...
    userBalances[key] = balance;
      ...
      if (balance > 0n) {
        ...
        if (cfg.adminUnlockRequired) {
        ...
        } else if (cfg.redemptionTokenAddress && cfg.withdrawTokenBalanceField) {
          // Generic ERC20 -> ETH redemption, e.g. Delphi: approve DEL to the
          // redemption contract, then call redeemTokens(raw DEL amount). The
          // ETH card amount is derived from the token balance by the index, but
          // the contract call must use token_balance_wei, not ETH wei.
          const apiE = apiBalances[key] || {};
          const rawTokenBalance = apiE[cfg.withdrawTokenBalanceField] || '0';
          const tokenBalance = BigInt(rawTokenBalance);
          const tokenDisplay = parseFloat(ethers.formatEther(tokenBalance)).toLocaleString('en', {
            minimumFractionDigits: 0,
            maximumFractionDigits: 4,
          });
          const tokenSym = cfg.redemptionTokenSymbol || 'token';
          const redemptionState = window._tokenRedemptionState?.[key];
          const liveTokenBal = redemptionState?.tokenBal;
          const tokenAllowance = redemptionState?.tokenAllowance || 0n;
          const needsApproval = tokenAllowance < tokenBalance;

          let actionBtn, stepInfo;
          if (!walletProvider) {
            actionBtn = `<button class="claim-btn" disabled style="opacity:0.5">Connect wallet to redeem</button>`;
            stepInfo = `<div class="claim-card-meta-row" style="margin-top:4px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.06)"><span class="claim-card-meta-label" style="color:#facc15">Connect wallet</span><span class="claim-card-meta-value" style="color:#facc15">Required to sign approve + redeem.</span></div>`;
          } else if (liveTokenBal !== undefined && liveTokenBal < tokenBalance) {
            actionBtn = `<button class="claim-btn" disabled style="opacity:0.5">Not enough ${esc(tokenSym)}</button><button class="claim-btn" disabled style="opacity:0.35">Step 2: Redeem ETH</button>`;
            stepInfo = `<div class="claim-card-meta-row" style="margin-top:4px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.06)"><span class="claim-card-meta-label" style="color:var(--red)">Blocked</span><span class="claim-card-meta-value" style="color:var(--red)">Wallet no longer holds the indexed ${esc(tokenDisplay)} ${esc(tokenSym)}.</span></div>`;
          } else if (needsApproval) {
            actionBtn = `<button class="claim-btn" id="claimBtn-${key}" data-action="token-redemption-approve" data-key="${key}">Step 1: Approve ${esc(tokenSym)}</button><button class="claim-btn" disabled style="opacity:0.35">Step 2: Redeem ETH</button>`;
            stepInfo = `<div class="claim-card-meta-row" style="margin-top:4px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.06)"><span class="claim-card-meta-label" style="color:#facc15">Status</span><span class="claim-card-meta-value" style="color:#facc15">Approves ${esc(tokenDisplay)} ${esc(tokenSym)}; Step 2 burns it for ${fmtEth(ethAmount)} ETH.</span></div>`;
          } else {
            actionBtn = `<button class="claim-btn" disabled style="opacity:0.35">Step 1: Approved</button><button class="claim-btn" id="claimBtn-${key}" data-action="token-redemption-redeem" data-key="${key}">Step 2: Redeem ETH</button>`;
            stepInfo = `<div class="claim-card-meta-row" style="margin-top:4px;padding-top:6px;border-top:1px solid rgba(255,255,255,0.06)"><span class="claim-card-meta-label" style="color:var(--green)">Ready</span><span class="claim-card-meta-value" style="color:var(--green)">${esc(tokenSym)} approved. Step 2 calls ${esc(cfg.withdrawCall)}(${esc(tokenSym)} amount).</span></div>`;
          }
          ...
          html += `
            <div class="claim-card">
              ...
              <div class="claim-card-meta" id="claimDetails-${key}">
                ${lastTx ? `<div class="claim-card-meta-row"><span class="claim-card-meta-label">Last tx</span><span class="claim-card-meta-value">${esc(lastTx.last_tx_date)} · <a href="${etherscanTx(lastTx.last_tx_hash)}" target="_blank" rel="noopener noreferrer">view tx</a></span></div>` : ''}
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Token</span><span class="claim-card-meta-value"><a href="${etherscanAddr(cfg.redemptionTokenAddress)}" target="_blank" rel="noopener noreferrer">${esc(tokenSym)}</a> · ${esc(tokenDisplay)} ${esc(tokenSym)}</span></div>
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Contract</span><span class="claim-card-meta-value"><a href="${etherscanAddr(cfg.contract)}" target="_blank" rel="noopener noreferrer">${cfg.contract}</a></span></div>
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Step 1</span><span class="claim-card-meta-value"><span style="color:var(--text)">approve(redemption contract, ${esc(tokenSym)} balance)</span></span></div>
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Step 2</span><span class="claim-card-meta-value"><span style="color:var(--text)">${esc(cfg.withdrawAbi.replace('function ', ''))}</span> using ${esc(tokenSym)} units</span></div>
                ${stepInfo}
              </div>
              <div class="claim-card-actions">
                ${actionBtn}
        ...
        } else if (cfg.dutchxMulti) {
          const dx = apiBalances[key]?.dutchx || {};
          const positions = dx.positions || [];
          const wethDirect = BigInt(dx.weth_direct_wei || '0');
          const saiDirect = BigInt(dx.sai_direct_wei || dx.sai_balance_wei || '0');
          const saiTotal = BigInt(dx.sai_balance_wei || saiDirect.toString());
          html += `
            <div class="claim-card">
              <div class="claim-card-header">
                <span class="claim-card-name">${esc(cfg.name)}</span>
                <span class="claim-card-amount">${fmtEth(ethAmount)} WETH${saiTotal > 0n ? ' + ' + esc(ethers.formatUnits(saiTotal, 18)) + ' SAI' : ''}</span>
              </div>
              <div class="claim-card-meta">
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Contract</span><span class="claim-card-meta-value"><a href="${etherscanAddr(cfg.contract)}" target="_blank" rel="noopener noreferrer">${cfg.contract}</a></span></div>
                <div class="claim-card-meta-row"><span class="claim-card-meta-label">Function</span><span class="claim-card-meta-value">withdraw(token, amount); positions first call claimBuyerFunds/claimSellerFunds(...)</span></div>
              </div>`;
          if (wethDirect > 0n) {
            html += `<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;display:flex;align-items:center;justify-content:space-between">
              <span style="font-size:13px">Direct WETH<span style="color:var(--text2);font-size:12px"> · ${fmtEth(ethers.formatEther(wethDirect))} WETH</span></span>
              <button class="claim-btn" data-action="dutchx-withdraw" data-key="${key}" data-token="WETH" data-amount-wei="${wethDirect.toString()}">Withdraw WETH</button>
            </div>`;
          }
          if (saiDirect > 0n) {
            html += `<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;display:flex;align-items:center;justify-content:space-between">
              <span style="font-size:13px">Direct SAI<span style="color:var(--text2);font-size:12px"> · ${esc(ethers.formatUnits(saiDirect, 18))} SAI</span></span>
              <button class="claim-btn" data-action="dutchx-withdraw" data-key="${key}" data-token="SAI" data-amount-wei="${saiDirect.toString()}">Withdraw SAI</button>
            </div>`;
          }
          for (let pi = 0; pi < positions.length; pi++) {
            const pos = positions[pi];
            const posSym = pos.received_symbol || 'WETH';
            const posAmt = pos.w !== undefined ? esc(String(pos.w)) : (pos.w_wei ? esc(ethers.formatUnits(BigInt(pos.w_wei), 18)) : '0');
            html += `<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;display:flex;align-items:center;gap:8px;justify-content:space-between;flex-wrap:wrap">
              <span style="font-size:13px">${esc(pos.t || 'position')} auction #${esc(String(pos.ai))}<span style="color:var(--text2);font-size:12px"> · ${posAmt} ${esc(posSym)}</span></span>
              <span style="display:flex;gap:6px;flex-wrap:wrap"><button class="claim-btn" data-action="dutchx-claim-position" data-key="${key}" data-position-idx="${pi}">Step 1: Claim</button><button class="claim-btn" data-action="dutchx-withdraw" data-key="${key}" data-token="${esc(posSym)}" data-amount-wei="${esc(String(pos.w_wei || '0'))}">Step 2: Withdraw</button></span>
            </div>`;
          }
          if (wethDirect === 0n && saiDirect === 0n && positions.length === 0) {
            html += `<div style="margin:8px 16px;font-size:12px;color:var(--text2)">DutchX balance details not available. <a href="${etherscanAddr(cfg.contract)}#writeContract" target="_blank" rel="noopener noreferrer">Use Etherscan</a>.</div>`;
          }
          html += `<div class="claim-card-status" id="claimStatus-${key}"></div></div>`;
...
async function claimETH(key) {
  ...
  try {
    ...
    if (cfg.withdrawTokenBalanceField) {
      const apiEntry = apiBalances[key] || {};
      const rawTokenBalance = apiEntry[cfg.withdrawTokenBalanceField];
      if (!rawTokenBalance) {
        throw new Error('Missing redemption token balance for withdrawal');
      }
      withdrawBalance = BigInt(rawTokenBalance);
    }
    ...
    if (cfg.redemptionTokenAddress && cfg.withdrawTokenBalanceField) {
      statusEl.textContent = 'Checking redemption token approval...';
      const tokenContract = new ethers.Contract(cfg.redemptionTokenAddress, [
        'function balanceOf(address) view returns (uint256)',
        'function allowance(address,address) view returns (uint256)',
        'function approve(address spender, uint256 amount) returns (bool)',
      ], walletSigner);
      const tokenBal = await tokenContract.balanceOf(walletAddress);
      if (tokenBal < withdrawBalance) {
        throw new Error('Wallet no longer holds enough ' + (cfg.redemptionTokenSymbol || 'redemption token'));
      }
      const allowance = await tokenContract.allowance(walletAddress, cfg.contract);
      if (allowance < withdrawBalance) {
        statusEl.textContent = 'Confirm ' + (cfg.redemptionTokenSymbol || 'token') + ' approval in wallet...';
        const approveTx = await tokenContract.approve(cfg.contract, withdrawBalance);
        statusEl.innerHTML = `<div style="padding:12px 16px;background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);margin-top:8px">
          <div style="font-size:12px;color:var(--text2);margin-bottom:4px">Approval submitted — waiting for confirmation...</div>
          <div style="font-family:monospace;font-size:12px;word-break:break-all"><a href="${etherscanTx(approveTx.hash)}" target="_blank" rel="noopener noreferrer">${approveTx.hash}</a></div>
        </div>`;
        logEvent('claim_started', { address: walletAddress, contract: key, extra: { step: 'token_approve', tx_hash: approveTx.hash } });
        await approveTx.wait();
        statusEl.textContent = 'Approval confirmed. Confirm redemption in wallet...';
      }
    }

    if (cfg.tokenApproval && cfg.tokenApprovalAbi && cfg.tokenAmountAbi) {
      statusEl.textContent = 'Reading token approval prerequisite...';
      const readContract = new ethers.Contract(cfg.contract, [cfg.tokenApprovalAbi, cfg.tokenAmountAbi], walletProvider || walletSigner);
      const tokenAddress = await readContract[cfg.tokenApproval.tokenCall]();
      const approvalAmount = await readContract[cfg.tokenApproval.amountCall](walletAddress);
      const spender = cfg.tokenApproval.spender === 'self' ? cfg.contract : cfg.tokenApproval.spender;
      if (approvalAmount > 0n) {
        statusEl.textContent = 'Confirm token approval prerequisite in wallet...';
        const tokenContract = new ethers.Contract(tokenAddress, ['function approve(address spender, uint256 amount) returns (bool)'], walletSigner);
        ...
        logEvent('claim_started', { address: walletAddress, contract: key, extra: { step: 'token_approve', tx_hash: approveTx.hash } });
        await approveTx.wait();
        statusEl.textContent = 'Approval confirmed. Confirm refund in wallet...';
      }
    }
...
function getTokenRedemptionAmount(key) {
  const cfg = EXCHANGES[key];
  const apiEntry = window._lastApiBalances?.[key] || {};
  const rawTokenBalance = apiEntry[cfg.withdrawTokenBalanceField];
  if (!rawTokenBalance) throw new Error('Missing redemption token balance — refresh and try again');
  return BigInt(rawTokenBalance);
}

async function tokenRedemptionApprove(key) {
  const cfg = EXCHANGES[key];
  const btn = document.getElementById('claimBtn-' + key);
  const statusEl = document.getElementById('claimStatus-' + key);
  if (!await checkNetwork()) { showInlineError('networkWarn', 'Please switch to Ethereum Mainnet.', 0); document.getElementById('networkWarn').classList.add('visible'); return; }
  ...
  btn.classList.add('pending');
  try {
    const amount = getTokenRedemptionAmount(key);
    const token = new ethers.Contract(cfg.redemptionTokenAddress, ['function balanceOf(address) view returns (uint256)', 'function approve(address,uint256) returns (bool)'], walletSigner);
    const tokenBal = await token.balanceOf(walletAddress);
    if (tokenBal < amount) throw new Error('Wallet no longer holds enough ' + (cfg.redemptionTokenSymbol || 'redemption token'));
    const tx = await token.approve(cfg.contract, amount);
    ...
    if (step2Btn) {
      ...
      step2Btn.style.opacity = '1';
      step2Btn.id = 'claimBtn-' + key;
      step2Btn.dataset.action = 'token-redemption-redeem';
      step2Btn.dataset.key = key;
      step2Btn.textContent = 'Step 2: Redeem ETH';
    }
    statusEl.innerHTML = '<span style="color:var(--green)">Approved. Click Step 2 to redeem for ETH.</span>';
    window.va?.track?.('token_redemption_approve_confirmed', { exchange: cfg.name, tx_hash: tx.hash });
  } catch (e) {
    btn.disabled = false;
    btn.textContent = 'Step 1: Approve ' + (cfg.redemptionTokenSymbol || 'token');
    btn.classList.remove('pending');
    if (e.code === 'ACTION_REJECTED' || e.code === 4001) {
      statusEl.textContent = 'Rejected';
    } else {
      statusEl.textContent = 'Approve failed: ' + (e.shortMessage || e.reason || e.message || 'Unknown error').slice(0, 150);
    }
  }
}

async function tokenRedemptionRedeem(key) {
  // claimETH already handles the actual redeem call using token_balance_wei and
  // re-checks token balance/allowance at click time. This wrapper gives the
  // explicit Step 2 button a protocol-specific action name.
  return claimETH(key);
}
...
const DUTCHX_SAI_ADDRESS = '0x89d24A6b4CcB1B6fAA2625fE562bDD9a23260359';

async function dutchxClaimPosition(key, posIdx, btn) {
  const cfg = EXCHANGES[key];
  const statusEl = document.getElementById('claimStatus-' + key);
  if (!await checkNetwork()) { showInlineError('networkWarn', 'Please switch to Ethereum Mainnet.', 0); document.getElementById('networkWarn').classList.add('visible'); return; }
  if (!walletSigner || !walletAddress) { showInlineError('walletError', 'Please connect your wallet first.'); return; }
  const positions = window._lastApiBalances?.[key]?.dutchx?.positions || [];
  const pos = positions[posIdx];
  if (!pos) { statusEl.textContent = 'DutchX position data missing — refresh and retry.'; return; }
  const abi = [
  ...
  const call = String(pos.t || '').toLowerCase() === 'buyer' ? 'claimBuyerFunds' : 'claimSellerFunds';
  btn.disabled = true;
  btn.classList.add('pending');
  btn.textContent = 'Claiming...';
  try {
    const contract = new ethers.Contract(cfg.contract, abi, walletSigner);
    const tx = await contract[call](pos.st, pos.bt, walletAddress, BigInt(pos.ai));
    statusEl.textContent = 'Waiting for confirmation...';
    await tx.wait();
    btn.textContent = '\u2713 Claimed';
    btn.classList.remove('pending');
    btn.classList.add('claimed');
    btn.disabled = true;
    const creditedSymbol = esc(pos.received_symbol || 'token');
    statusEl.innerHTML = '<span style="color:var(--green)">Auction #' + esc(String(pos.ai)) + ' claimed. Now withdraw the credited ' + creditedSymbol + '.</span> <a href="' + etherscanTx(tx.hash) + '" target="_blank" rel="noopener noreferrer">View tx</a>';
    logEvent('claim_started', { address: walletAddress, contract: key, extra: { step: 'dutchx_claim_position', tx_hash: tx.hash, auction_index: pos.ai } });
  } catch (e) {
    btn.disabled = false;
    btn.classList.remove('pending');
    btn.textContent = 'Step 1: Claim';
    statusEl.textContent = (e.code === 'ACTION_REJECTED' || e.code === 4001) ? 'Rejected' : 'Claim failed: ' + (e.shortMessage || e.reason || e.message || 'Unknown error').slice(0, 150);
  }
}

async function dutchxWithdraw(key, tokenSymbol, amountWei, btn) {
  const cfg = EXCHANGES[key];
  const statusEl = document.getElementById('claimStatus-' + key);
  if (!await checkNetwork()) { showInlineError('networkWarn', 'Please switch to Ethereum Mainnet.', 0); document.getElementById('networkWarn').classList.add('visible'); return; }
  if (!walletSigner || !walletAddress) { showInlineError('walletError', 'Please connect your wallet first.'); return; }
  const token = tokenSymbol === 'SAI' ? DUTCHX_SAI_ADDRESS : WETH9_ADDRESS;
  const unit = tokenSymbol === 'SAI' ? 'SAI' : 'WETH';
  const amt = BigInt(amountWei || '0');
  if (amt <= 0n) { statusEl.textContent = 'Nothing to withdraw for this item.'; return; }
  btn.disabled = true;
  btn.classList.add('pending');
  btn.textContent = 'Withdrawing...';
  try {
    const contract = new ethers.Contract(cfg.contract, ['function withdraw(address token, uint256 amount)'], walletSigner);
    const tx = await contract.withdraw(token, amt);
    statusEl.textContent = 'Waiting for confirmation...';
    await tx.wait();
    ...
    btn.classList.add('claimed');
    btn.disabled = true;
    const amtDisplay = ethers.formatUnits(amt, 18);
    buildRecoveredStatus(statusEl, 'Withdrawn from DutchX', amtDisplay, unit, tx.hash);
    if (unit === 'WETH') showDonationModal(parseFloat(amtDisplay), key, { returnsWeth: true });
    logEvent('claim_started', { address: walletAddress, contract: key, extra: { step: 'dutchx_withdraw', token: unit, tx_hash: tx.hash } });
  } catch (e) {
    btn.disabled = false;
    btn.classList.remove('pending');
    btn.textContent = tokenSymbol === 'SAI' ? 'Withdraw SAI' : 'Withdraw WETH';
    statusEl.textContent = (e.code === 'ACTION_REJECTED' || e.code === 4001) ? 'Rejected' : 'Withdraw failed: ' + (e.shortMessage || e.reason || e.message || 'Unknown error').slice(0, 150);
  }
}
  ...
  for (const { key, balance } of results) {
    ...
    if (balance > 0n) {
      ...
      if (cfg.dutchxMulti) {
        actionableFound++;
        const apiEntry = apiBalances[key] || {};
        const dx = apiEntry.dutchx || {};
        const positions = dx.positions || [];
        const wethDirect = BigInt(dx.weth_direct_wei || '0');
        const saiDirect = BigInt(dx.sai_direct_wei || dx.sai_balance_wei || '0');
        const saiTotal = BigInt(dx.sai_balance_wei || saiDirect.toString());
        const summaryParts = [];
        if (wethDirect > 0n) summaryParts.push(fmtEth(ethers.formatEther(wethDirect)) + ' WETH');
        if (saiTotal > 0n) summaryParts.push(parseFloat(ethers.formatUnits(saiTotal, 18)).toLocaleString('en', { maximumFractionDigits: 4 }) + ' SAI');
        if (summaryParts.length === 0) summaryParts.push(fmtEth(ethAmount) + ' WETH');
        html += '<div class="claim-card"><div class="claim-card-header"><span class="claim-card-name">' + esc(cfg.name) + mLastTxHtml + '</span><span class="claim-card-amount">' + esc(summaryParts.join(' + ')) + '</span><span class="claim-card-tag">Claimable</span></div>' +
          '<div class="claim-card-meta"><div class="claim-card-meta-row"><span class="claim-card-meta-label">Contract</span><span class="claim-card-meta-value"><a href="' + etherscanAddr(cfg.contract) + '" target="_blank" rel="noopener noreferrer">' + cfg.contract + '</a></span></div>' +
          '<div class="claim-card-meta-row"><span class="claim-card-meta-label">Function</span><span class="claim-card-meta-value">withdraw(token, amount); positions first call claimBuyerFunds/claimSellerFunds(...)</span></div></div>';
        if (wethDirect > 0n) {
          html += '<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;font-size:13px">Direct WETH<span style="color:var(--text2);font-size:12px"> · ' + fmtEth(ethers.formatEther(wethDirect)) + ' WETH</span></div>';
        }
        if (saiDirect > 0n) {
          html += '<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;font-size:13px">Direct SAI<span style="color:var(--text2);font-size:12px"> · ' + parseFloat(ethers.formatUnits(saiDirect, 18)).toLocaleString('en', { maximumFractionDigits: 4 }) + ' SAI</span></div>';
        }
        for (var dxi = 0; dxi < positions.length; dxi++) {
          var pos = positions[dxi];
          var posSym = pos.received_symbol || 'WETH';
          var posAmt = pos.w !== undefined ? String(pos.w) : (pos.w_wei ? ethers.formatUnits(BigInt(pos.w_wei), 18) : '0');
          html += '<div class="claim-row" style="margin:4px 16px;border-left:2px solid var(--accent);padding:6px 12px;font-size:13px">' + esc(pos.t || 'position') + ' auction #' + esc(String(pos.ai)) + '<span style="color:var(--text2);font-size:12px"> · ' + esc(posAmt) + ' ' + esc(posSym) + '</span></div>';
        }
        html += '<div style="margin:8px 16px 4px;font-size:12px;color:var(--text2)">Connect this wallet to claim auction positions and withdraw WETH/SAI.</div></div>';
        continue;
      }
      ...
      if (cfg.redemptionTokenAddress && cfg.withdrawTokenBalanceField) {
        actionableFound++;
        const apiEntry = apiBalances[key] || {};
        const rawTokenBalance = apiEntry[cfg.withdrawTokenBalanceField] || '0';
        let tokenDisplay = rawTokenBalance;
        try {
          tokenDisplay = parseFloat(ethers.formatEther(BigInt(rawTokenBalance))).toLocaleString('en', { maximumFractionDigits: 4 });
        } catch (_) {}
        const tokenSym = cfg.redemptionTokenSymbol || 'token';
        html += '<div class="claim-card"><div class="claim-card-header"><span class="claim-card-name">' + esc(cfg.name) + mLastTxHtml + '</span><span class="claim-card-amount">' + fmtEth(ethAmount) + ' ETH</span><span class="claim-card-tag">Claimable</span></div>' +
          '<div class="claim-card-meta" id="claimDetails-' + key + '">' +
            ...
            '<div class="claim-card-meta-row"><span class="claim-card-meta-label">Step 1</span><span class="claim-card-meta-value"><span style="color:var(--text)">approve(redemption contract, ' + esc(tokenSym) + ' balance)</span></span></div>' +
            '<div class="claim-card-meta-row"><span class="claim-card-meta-label">Step 2</span><span class="claim-card-meta-value"><span style="color:var(--text)">' + esc(cfg.withdrawAbi.replace('function ', '')) + '</span> using ' + esc(tokenSym) + ' units</span></div>' +
          '</div><div style="margin:8px 16px 4px;font-size:12px;color:var(--text2)">Connect this wallet to approve ' + esc(tokenSym) + ', then redeem for ETH.</div></div>';
        continue;
      }
      actionableFound++;
...
document.getElementById('claimBanner').addEventListener('click', function(e) {
  ...
  } else if (action === 'token-redemption-approve') {
    tokenRedemptionApprove(btn.dataset.key);
  } else if (action === 'token-redemption-redeem') {
    tokenRedemptionRedeem(btn.dataset.key);
  ...
  } else if (action === 'dutchx-claim-position') {
    dutchxClaimPosition(btn.dataset.key, parseInt(btn.dataset.positionIdx), btn);
  } else if (action === 'dutchx-withdraw') {
    dutchxWithdraw(btn.dataset.key, btn.dataset.token, btn.dataset.amountWei, btn);
